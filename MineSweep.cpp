/*
 * @Author: junyu33 
 * @Date: 2021-12-14 16:58:52 
 * @Last Modified by: junyu33
 * @Last Modified time: 2021-12-14 17:14:30
 */
...
void Block::render()
{
	...
	case FOUND: 
		...
		{
			char s[64];
			switch (m_num)// ver 3.2
			{
				case 1: setcolor(BLUE); break;
				case 2: setcolor(GREEN); break;
				case 3: setcolor(RED); break;
				case 4: setcolor(0x000080); break;//dark blue
				case 5: setcolor(0x8B4513); break;//brown
				case 6: setcolor(0x00FFFF); break;//cyan
				case 7: setcolor(0x800080); break;//purple
			}
			sprintf(s, "%d", m_num);
			outtextxy(m_x, m_y, s);
		}
...
void MineArray::loadResource()
{
	...
	m_imgSpace[1] = newimage(); //ver 3.1

	getimage(m_imgSpace[0],"space.bmp");
	getimage(m_imgSpace[1],"space_chosen.bmp");//ver 3.1
...
{
	if(x < 0 || x >= m_width || y < 0 || y >= m_height)//边界判断
		return;
	BlockType tp = m_block[x][y]->getType();
	if(m_block[x][y]->getNum() == 0 && tp == NORMAL)//如果这个方块周围都没有雷
	{
		m_block[x][y]->setType(FOUND);//状态就确定了
		--m_blockNum;
		findMore(x-1,y);
		findMore(x,y-1);
		findMore(x-1, y-1);
		...
		findMore(x,y+1);
		findMore(x-1, y+1);
		findMore(x+1, y+1);//递归寻找周围八个方块的状态
	}
	else if(m_block[x][y]->getNum() > 0 && tp != FOUND)//如果周围方块有雷
	{
		m_block[x][y]->setType(FOUND);//只标记它的状态确定
		--m_blockNum;
	}
}
...
{
	PIXEL2BLOCK(x, y);//将坐标转化为方块的位置

	if(m_block[x][y]->isMine() && m_block[x][y]->getType() != FLAG)//如果点到了雷，而且没有被标旗
	{
		m_firedX = x;
		m_firedY = y;
		return false;//你就踩到雷了
	...
	switch(m_block[x][y]->getType())//当前选定的方块处于什么状态
	{
	case NORMAL:
	case CHOSEN:
	case UNKNOWN:
	case UNKNOWN_CHOSEN://如果这个方块状态不确定，而且被选了
		findMore(x, y);//查找该方块的状态
	default:;
	}
...
void MineArray::moveFrame(int x, int y, bool isdirection)
{
	PIXEL2BLOCK(x, y);
	...
	{
	case NORMAL:
		putimage(a, b, m_imgBlock[isdirection]);
		break;
	case FLAG:
		putimage(a, b, m_imgFlag[isdirection]); break;
	case UNKNOWN:
		putimage(a, b, m_imgUnknown[isdirection]); break;
	case FOUND://ver 2.2
		if(m_block[x][y]->m_num != 0)
		{
			putimage(a, b, m_imgSpace[0]); //ver 3.1
			char s[64];
			setcolor(YELLOW);
			setfont(35, 20, "consolas");
			sprintf(s, "%d", m_block[x][y]->m_num);
			outtextxy(a, b, s);
		}
		else 
		{
			putimage(a, b, m_imgSpace[isdirection]); //ver 3.1
		}
		break;
	default:;
	}
...
void MineArray::flagall(int x, int y)
{
	PIXEL2BLOCK(x,y);
	if(m_block[x][y]->getType() != FOUND)
		return;
...
void MineArray::winFrame()
{
	int LEN=960,WID=640;
	initgraph(LEN, WID);
	float x = LEN / 2, y = WID / 2;
	//float x = WID, y = LEN;	
	cleardevice();
	while(kbhit()) getch();
	int timeend = time(0);//ver 2.1
	for(float dx = RANDF, dy = RANDF;; x += dx, y += dy)
	{
		...
		PIMAGE img;//ver2.2
		img=newimage();
		getimage(img,"R.png",0,0);
		putimage(0,0,LEN,WID,img,0,0,960,640);
		xyprintf(x-150, y-50, "You Win! You finished in %d seconds", timeend - timestp, 50);//ver 2.1
...
void MineArray::loseFrame()
{
	...
	while(kbhit()) getch();
 	for(int x, y; ; delay_fps(60))
 	{
		...
		float xx = g_scrWidth / 2, yy = g_scrHeight / 2;
		setfont(20,10,"consolas");
		setcolor(YELLOW);
		outtextxy(xx, yy-50, "You Lose!");
		outtextxy(xx-100, yy+50, "Press anykey to continue!");
